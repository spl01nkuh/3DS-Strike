...
void mmDebWriteTag(s8* tag) {
    /* No-op. The debug print that consumed `tag` is disabled, so this does
     * nothing useful — but the original loop rewrote '\n'->' ' IN PLACE, i.e.
     * it wrote into the caller's buffer. Callers pass string LITERALS (e.g.
     * Warning_Init's mmDebWriteTag("\nWARNING\n\n")), which live in read-only
     * memory: the write faults with a data abort on real hardware (the MMU
     * enforces .rodata; Azahar does not). That was the boot crash. */
    (void)tag;
}
