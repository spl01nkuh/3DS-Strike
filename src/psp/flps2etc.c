...
#include "AcrSDK/common/plapx.h"
#include "AcrSDK/common/plbmp.h"
#include "AcrSDK/common/plcommon.h"
#include "AcrSDK/common/plpic.h"
#include "AcrSDK/common/pltim2.h"
/*
...
void __attribute__((noinline)) memcpy_vfpu_simple(void *dst, void *src, size_t size) {
    /* VFPU removed for 3DS (only referenced from dead code) */
    memcpy(dst, src, size);
}
...
u32 flPS2GetSystemMemoryHandle(s32 len, s32 type) {
    ...
    if (handle == 0) {
        ...
        if (handle == 0) {
            ...
            flLogOut("ERROR flPS2GetSystemMemoryHandle flps2etc.c\n");
            while (1) {}
        }
    }
...
void flCompact() {
    /* mflCompact() (plmemCompact) physically memmove()s allocated blocks to
     * close gaps — any C3D_Tex the GPU-side cache built from a CPU source
     * pointer can go stale the instant compaction relocates that memory.
     * The cache is keyed by raw pointer with no relocation hook, so a freed
     * old address can silently get reused by unrelated data afterward,
     * making a cached entry alias into a completely different texture —
     * exactly the "chunks of wrong/unrelated color" corruption seen in the
     * opening montage, which allocates 91 textures + palettes in one burst
     * (the likeliest place to exhaust/fragment system memory and trigger
     * this path). Full flush is correct and cheap relative to how rarely
...
uintptr_t flPS2GetSystemTmpBuff(s32 len, s32 align) {
    ...
    if (flPs2State.SystemTmpBuffEndAdrs < new_now) {
        flPrintColor(0xFFFF0000);
        flLogOut("ERROR flPS2GetSystemTmpBuff flps2etc.c\n");
