...
#include "Game/EFFA0.h"
#include "Game/EFFA3.h"
#include "Game/EFFECT.h"
#include "Game/EX_DATA.h"
...
/* Maps each on-screen row (top to bottom) to its Shot[] slot. Slots are owned
 * by (X,Y,L,ZR,A,B,R,ZL) -- measured, see sf3_btn_label() in sc_sub.c -- so
 * these indices make the rows read Y, X, L, R, B, A, ZL, ZR. Used here for
 * editing and in EFF10.c/EFF23.c for the glyph and function name, so all three
 * agree. Rows 8/9/10 = vibration/default/exit bypass this. */
