...
#include "Game/Eff39.h"
#include "Game/Eff43.h"
#include "Game/Eff76.h"
#include "Game/EffA9.h"
#include "Game/EffE0.h"
#include "Game/EffK6.h"
...
void Setup_PL_Color(s16 PL_id, u16 sw) {
     ...
     * LK=0x010 MK=0x020 HK=0x040). User-verified-empirically-in-game mapping
     * (odd/even pair swap relative to the naive LP=0,MP=1,HP=2,LK=3,MK=4,HK=5
     * grouping — that grouping still showed punch presses producing kick
     * colors on real hardware, this pairwise-swapped one is confirmed by the
     * user to match): LP->1 MP->0 HP->3 LK->2 MK->5 HK->4, each button's
     * "already taken, use alternate" pair is its same-strength opposite
     * type (LP<->LK, MP<->MK, HP<->HK). Do not re-derive from pad.c bit
     * layout alone — that reasoning already failed once. */
    if (sw_new & 0x4000) {
        switch (sw) {
        case 256: /* LP */
